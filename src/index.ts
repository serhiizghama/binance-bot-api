...
import { ITicker24hr, ITickerPrice, USDT } from './interface';
dotenv.config();
...
bot.on('message', async msg => {
    ...
    const tickerPrice = await helper.getBinanceTickerPrice();
    const ticker24hr = await helper.getBinanceTicker24hr();

    const friendlyUserMessage = userMessage.toLowerCase();
    if (friendlyUserMessage === USDT) return;

    for (const pair of tickerPrice) {
        const friendlySymbol = pair.symbol.toLowerCase();
        const price = pair.price;

        if (friendlySymbol.includes(USDT)) {
            for (const ticker of ticker24hr) {
                if (friendlySymbol.includes(friendlyUserMessage)
                    && friendlySymbol.includes(USDT)
                    && friendlyUserMessage.length + USDT.length === friendlySymbol.length) {

                    const priceChange = +ticker.priceChange;
                    const volume = +ticker.volume;

                    bot.sendMessage(chatId, `${userMessage.toUpperCase()} цена: ${Math.round(price * 100) / 100}$ \nОбъем за 24 часа: ${volume} монет\nИзменение цены за 24 часа: ${priceChange}\n`);
                    break;
                }
            }
        }
...
export namespace helper {
    ...
    export function getBinanceTickerPrice(): Promise<ITickerPrice[]> {
        return new Promise((resolve, reject) => {
            request(`${binanceApi}/ticker/price`, (error: any, _response: any, body: string) => {
                if (error) {
                    reject(error);
                }

                const tickerPrices: ITickerPrice[] = JSON.parse(body);
                resolve(tickerPrices);
            });
        });
    };

    export function getBinanceTicker24hr(): Promise<ITicker24hr[]> {
        return new Promise((resolve, reject) => {
            request(`${binanceApi}/ticker/24hr`, (error, response, body) => {
                if (error) {
                    reject(error);
                }

                const ticker24hr: ITicker24hr[] = JSON.parse(body);
                resolve(ticker24hr);
            });
        });
    };
